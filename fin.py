...
try:
    from fastapi import FastAPI, WebSocket, WebSocketDisconnect, HTTPException, Depends, status, File, UploadFile
    ...
    import random
    import numpy as np
    from sqlalchemy import create_engine, Column, String, Float, DateTime, Enum as SQLAlchemyEnum, text, LargeBinary
...
# Serve frontend HTML at root endpoint
...
async def serve_frontend():
    ...
    return HTMLResponse(content="<h1>MT5 Simulator API</h1><p>Frontend not found. API is running at /docs</p>")
...
def get_database_url(account_type: str) -> str:
    # Sanitize account type for filename (replace / with _)
    safe_account_type = account_type.lower().replace('/', '_')
    # In production (Render), use absolute path for SQLite
    if os.getenv('RENDER'):
        db_path = os.path.join(os.getcwd(), f'trades_{safe_account_type}.db')
        return f"sqlite:///{db_path}"
    # For local development
    return f"sqlite:///trades_{safe_account_type}.db"
...
# Constants
CURRENCY_PAIRS = ["EURUSD", "GBPUSD", "USDJPY", "USDCNH", "USDRUB", "AUDUSD", "NZDUSD", "USDSEK", "XAUUSD", "XAUAUD", "USDCAD"]
DEFAULT_ACCOUNT_BALANCE = 10000.0
DEFAULT_LEVERAGE = 500
SWAP_RATE_BUY = -0.0001
SWAP_RATE_SELL = 0.00005
COMMISSION = 0.0001
ACCOUNT_TYPES = ["Fast/Acc", "Demo/Acc", "Hunter/Acc", "LivePro/Acc"]

# Stable account identifiers (9 digits) for external identification
ACCOUNT_ID_MAP: Dict[str, str] = {
    "Fast/Acc": "410007254",
    "Demo/Acc": "297738583",
    "Hunter/Acc": "410000129",
    "LivePro/Acc": "223253111",
}

# Global session state
LICENSE_KEY = "1234"
session_state = {
    "current_account": "Fast/Acc",
...
# Database model for Profile Images
class ProfileImageDB(Base):
    __tablename__ = "profile_images"

    id = Column(String, primary_key=True, index=True)
    category = Column(String, nullable=False, index=True)  # 'chats', 'messages', or 'quotes'
...
def get_profile_database_url() -> str:
    if os.getenv('RENDER'):
        db_path = os.path.join(os.getcwd(), 'profile.db')
        return f"sqlite:///{db_path}"
...
print(f"Profile database initialized at {profile_db_url}")

def get_db(account_type: str = "Fast/Acc"):
...
def load_account_balances():
    ...
    try:
        ...
        print(f"WARNING: Could not load balances: {e}")
...
for account_type in ACCOUNT_TYPES:
    currency_pair_settings_store[account_type] = {
        "EURUSD": CurrencyPairConfig(symbol="EURUSD", buy_starting_price=1.1726, sell_starting_price=1.1727, volatility=0.00005, spread=0.0002, buy_lot_size=0.01, sell_lot_size=0.01, default_target_profit=10.0, default_target_loss=5.0, pip_value=0.0001, mean_reversion_strength=0.05),
        "GBPUSD": CurrencyPairConfig(symbol="GBPUSD", buy_starting_price=1.3577, sell_starting_price=1.3579, volatility=0.00005, spread=0.0003, buy_lot_size=0.01, sell_lot_size=0.01, default_target_profit=10.0, default_target_loss=5.0, pip_value=0.0001, mean_reversion_strength=0.05),
        "USDJPY": CurrencyPairConfig(symbol="USDJPY", buy_starting_price=147.1960, sell_starting_price=147.2060, volatility=0.00005, spread=0.02, buy_lot_size=0.01, sell_lot_size=0.01, default_target_profit=100.0, default_target_loss=50.0, pip_value=0.01, mean_reversion_strength=0.0004),
        "USDCNH": CurrencyPairConfig(symbol="USDCNH", buy_starting_price=7.1823, sell_starting_price=7.1837, volatility=0.00005, spread=0.0020, buy_lot_size=0.01, sell_lot_size=0.01, default_target_profit=50.0, default_target_loss=25.0, pip_value=0.0001, mean_reversion_strength=0.05),
        "USDRUB": CurrencyPairConfig(symbol="USDRUB", buy_starting_price=84.9900, sell_starting_price=85.0400, volatility=0.00005, spread=0.02, buy_lot_size=0.01, sell_lot_size=0.01, default_target_profit=500.0, default_target_loss=250.0, pip_value=0.01, mean_reversion_strength=0.0007),
        "AUDUSD": CurrencyPairConfig(symbol="AUDUSD", buy_starting_price=0.6639, sell_starting_price=0.6641, volatility=0.00005, spread=0.0002, buy_lot_size=0.01, sell_lot_size=0.01, default_target_profit=10.0, default_target_loss=5.0, pip_value=0.0001, mean_reversion_strength=0.05),
        "NZDUSD": CurrencyPairConfig(symbol="NZDUSD", buy_starting_price=0.5975, sell_starting_price=0.5977, volatility=0.00005, spread=0.0002, buy_lot_size=0.01, sell_lot_size=0.01, default_target_profit=10.0, default_target_loss=5.0, pip_value=0.0001, mean_reversion_strength=0.05),
        "USDSEK": CurrencyPairConfig(symbol="USDSEK", buy_starting_price=9.3673, sell_starting_price=9.3873, volatility=0.00005, spread=0.02, buy_lot_size=0.01, sell_lot_size=0.01, default_target_profit=100.0, default_target_loss=50.0, pip_value=0.01, mean_reversion_strength=0.05),
        "XAUUSD": CurrencyPairConfig(symbol="XAUUSD", buy_starting_price=2650.00, sell_starting_price=2650.50, volatility=0.00005, spread=0.50, buy_lot_size=0.01, sell_lot_size=0.01, default_target_profit=10.0, default_target_loss=5.0, pip_value=0.01, mean_reversion_strength=0.05),
        "XAUAUD": CurrencyPairConfig(symbol="XAUAUD", buy_starting_price=4100.00, sell_starting_price=4100.80, volatility=0.00005, spread=0.80, buy_lot_size=0.01, sell_lot_size=0.01, default_target_profit=10.0, default_target_loss=5.0, pip_value=0.01, mean_reversion_strength=0.05),
        "USDCAD": CurrencyPairConfig(symbol="USDCAD", buy_starting_price=1.3580, sell_starting_price=1.3582, volatility=0.00005, spread=0.0002, buy_lot_size=0.01, sell_lot_size=0.01, default_target_profit=10.0, default_target_loss=5.0, pip_value=0.0001, mean_reversion_strength=0.05)
    }
...
def generate_mt5_price(previous_price: float, volatility: float, mean_price: float, 
                      ...
                      symbol: str, entry_price: float = None, target_amount: float = 0.0) -> float:
    """
    Generate realistic MT5 price simulation with directional oscillation toward target.
    
    Key Features:
    1. Initial directional drift determined by target_amount and target_type
    2. Oscillation ratio 5:2 (forward:backward) before bias - mixed manner
    3. Oscillation ratio 6:2 (forward:backward) after bias - mixed manner
    4. Forward = toward target (profit or loss), Backward = away from target
    
    Real MT5 Behavior:
    - Prices oscillate toward target based on target_type (PROFIT or LOSS)
    - When bias triggered, stronger forward tendency (6:2 ratio) + faster movement
    - Oscillations are mixed (not sequential forward then backward)
    """
    # Get pip value for this symbol
    pip_value = get_pip_value_for_symbol(symbol)
    
    # ========================================================================
    # DETERMINE DIRECTIONAL DRIFT BASED ON TARGET_TYPE AND TARGET_AMOUNT
    # ========================================================================
    # For PROFIT target: price should move to increase P&L (toward target_price)
    # For LOSS target: price should move to decrease P&L (toward target_price for loss)
    # The target_price already encodes the direction based on target_type
    
    # Calculate which direction is "forward" (toward achieving the target)
    if target_type == TargetType.PROFIT:
        # For profit: move toward higher P&L
        if trade_direction == TradeDirection.BUY:
            # BUY trade profits when price goes up
            direction_to_target = 1 if target_price > previous_price else -1
        else:  # SELL
            # SELL trade profits when price goes down
            direction_to_target = -1 if target_price < previous_price else 1
    else:  # TargetType.LOSS
        # For loss: move toward lower P&L (more negative)
        if trade_direction == TradeDirection.BUY:
            # BUY trade loses when price goes down
            direction_to_target = -1 if target_price < previous_price else 1
        else:  # SELL
            # SELL trade loses when price goes up
            direction_to_target = 1 if target_price > previous_price else -1
    
    # ========================================================================
    # OSCILLATION RATIO: 5:2 (no bias) or 6:2 (with bias) - MIXED MANNER
    # ========================================================================
    if bias_factor > 0:
        # Biased: 6:2 ratio = 6/(6+2) = 75% chance of moving forward
        forward_probability = 6.0 / 8.0
    else:
        # Normal: 5:2 ratio = 5/(5+2) ≈ 71.4% chance of moving forward
        forward_probability = 5.0 / 7.0
    
    # Randomly determine if this tick moves forward or backward (MIXED pattern)
    is_forward = np.random.random() < forward_probability
    movement_direction = direction_to_target if is_forward else -direction_to_target
    
    # ========================================================================
    # MOVEMENT MAGNITUDE: How much to move per tick
    # ========================================================================
    if bias_factor > 0:
        # Biased: larger movements for faster target achievement
        base_magnitude = volatility * 0.25
        # Additional drift component when biased (boost toward target)
        distance_ratio = abs(target_price - previous_price) / previous_price
        drift_boost = min(distance_ratio * 0.1, 0.002)  # Cap at 0.2%
        movement_magnitude = base_magnitude + drift_boost
    else:
        # Normal: standard movement magnitude
        base_magnitude = volatility * 0.2
        movement_magnitude = base_magnitude
    
    # Add some randomness to magnitude (not just direction)
    magnitude_variation = np.random.uniform(0.5, 1.5)
    final_magnitude = movement_magnitude * magnitude_variation
    
    # ========================================================================
    # CALCULATE PRICE CHANGE
    # ========================================================================
    price_change = movement_direction * final_magnitude
    new_price = previous_price * (1 + price_change)
    
    # ========================================================================
    # BOUNDS: Limit maximum change per tick (realistic MT5 behavior)
    # ========================================================================
    max_pips = 3 if bias_factor == 0 else 5
    max_change = max_pips * pip_value
    
    new_price = max(previous_price - max_change, min(previous_price + max_change, new_price))
...
async def account_simulate_trades(account_type: str):
    ...
    while True:
        ...
        for trade_id, trade in list(active_trades.items()):
            ...
            # Pass entry_price so price oscillates around it (realistic back-and-forth P&L)
            new_buy_price = generate_mt5_price(
                ...
                symbol=trade.symbol,
                entry_price=trade.entry_price,  # Oscillate around entry price for realistic P&L fluctuation
                target_amount=trade.target_amount  # Consider target amount for directional drift
            )
        ...
        # Broadcast real-time Exness stats after each simulation cycle
        await broadcast_exness_stats(account_type)
        ...
        with SessionLocal() as db:
            for tid, trade in completed_trades:
                try:
                    ...
                    print(f"WARNING: Error processing trade {tid}: {e}")
            ...
            try:
                ...
                db.rollback()
                print(f"WARNING: Database commit failed: {commit_error}")
                return  # Don't remove trades if commit failed
            
    except Exception as e:
        print(f"WARNING: Error transferring completed trades to database for {account_type}: {e}")
...
async def auto_save_trades_to_database(account_type: str):
    ...
    while True:
        ...
        try:
            ...
            print(f"WARNING: Error auto-saving trades for {account_type}: {e}")
...
async def update_account_metrics(account_type: str):
    ...
    account_metrics.equity = account_metrics.balance + total_pnl + account_metrics.deposit
...
async def root():
    ...
    # Fallback to API info page if frontend.html not found
    ...
    <html>
        ...
        <body>
            ...
            <ul>
                <li><strong>Current Account:</strong> {session_state.get("current_account", "Fast/Acc")}</li>
            </ul>
...
async def test_connection():
    """Simple endpoint to test Android app connectivity"""
    return {
        "status": "success",
        "message": "Android app successfully connected to backend!",
        "server_time": get_current_time_in_timezone().strftime("%Y-%m-%d %H:%M:%S"),
        "available_accounts": ACCOUNT_TYPES,
        "base_url": "http://192.168.10.195:8080/"
    }
...
async def list_accounts():
    ...
    for account_type in ACCOUNT_TYPES:
        ...
        accounts_info[account_type] = {
            "account_id": ACCOUNT_ID_MAP.get(account_type),
    ...
    return {
        ...
        "current_account": session_state.get("current_account", "Fast/Acc"),
        "current_account_id": ACCOUNT_ID_MAP.get(session_state.get("current_account", "Fast/Acc")),
...
@app.get("/api/sum-in-exness")
async def sum_in_exness():
    """
    Returns aggregated statistics for each currency pair:
    - Count of active running trades
    - Continuous sum of profits/losses for running trades
    - Lowest and highest entry prices of active trades
    - Count of completed trades today
    """
    account_type = session_state.get("current_account", "Fast/Acc")
    stats = calculate_exness_stats(account_type)
    
    return {
        "account_type": account_type,
        "currency_pair_stats": stats,
        "timestamp": get_current_time_in_timezone().strftime(f"%Y-%m-%d %H:%M:%S {SYSTEM_TIMEZONE}"),
        "message": "Sum in Exness statistics retrieved successfully"
    }

def calculate_exness_stats(account_type: str, include_completed_today: bool = True):
    """
    Helper function to calculate real-time Exness stats for an account.
    Uses CURRENT prices (not entry prices) for lowest/highest tracking.
    """
    active_trades = active_trades_store[account_type]
    
    # Initialize result structure for each currency pair
    result = {}
    for symbol in CURRENCY_PAIRS:
        result[symbol] = {
            "active_trade_count": 0,
            "running_profit_loss_sum": 0.0,
            "lowest_current_price": None,
            "highest_current_price": None,
            "completed_trades_today": 0
        }
    
    # Process active trades - use CURRENT prices (fluctuating), not entry prices
    for trade in active_trades.values():
        if trade.status == TradeStatus.RUNNING:
            symbol = trade.symbol
            if symbol in result:
                result[symbol]["active_trade_count"] += 1
                result[symbol]["running_profit_loss_sum"] += trade.profit_loss
                
                # Get current price based on trade direction
                current_price = trade.current_buy_price if trade.trade_direction == TradeDirection.BUY else trade.current_sell_price
                
                # Track lowest and highest CURRENT prices (these fluctuate in real-time)
                if result[symbol]["lowest_current_price"] is None or current_price < result[symbol]["lowest_current_price"]:
                    result[symbol]["lowest_current_price"] = current_price
                if result[symbol]["highest_current_price"] is None or current_price > result[symbol]["highest_current_price"]:
                    result[symbol]["highest_current_price"] = current_price
    
    # Count completed trades today from database (only if requested)
    if include_completed_today:
        today = get_current_time_in_timezone().replace(hour=0, minute=0, second=0, microsecond=0, tzinfo=None)
        SessionLocal = account_session_makers[account_type]
        with SessionLocal() as db:
            for symbol in CURRENCY_PAIRS:
                count = db.query(TradeDataDB).filter(
                    TradeDataDB.symbol == symbol,
                    TradeDataDB.status.in_([TradeStatus.COMPLETED, TradeStatus.STOPPED]),
                    TradeDataDB.end_time >= today
                ).count()
                result[symbol]["completed_trades_today"] = count
    
    # Round floating point values
    for symbol in result:
        result[symbol]["running_profit_loss_sum"] = round(result[symbol]["running_profit_loss_sum"], 2)
        if result[symbol]["lowest_current_price"] is not None:
            result[symbol]["lowest_current_price"] = round(result[symbol]["lowest_current_price"], 5)
        if result[symbol]["highest_current_price"] is not None:
            result[symbol]["highest_current_price"] = round(result[symbol]["highest_current_price"], 5)
    
    return result


async def broadcast_exness_stats(account_type: str):
    """Broadcast real-time Exness stats via WebSocket"""
    stats = calculate_exness_stats(account_type, include_completed_today=False)
    
    await manager.broadcast(json.dumps({
        "type": "exness_stats_update",
        "account_type": account_type,
        "currency_pair_stats": stats,
        "timestamp": get_current_time_in_timezone().strftime(f"%Y-%m-%d %H:%M:%S {SYSTEM_TIMEZONE}")
    }))

@app.get("/api/profits-losses-by-date")
async def get_profits_losses_by_date():
    """
    Returns all profits and losses attained by completed trades, grouped by date in chronological order.
    This is recalculated whenever admin updates completed trades in the database.
    """
    account_type = session_state.get("current_account", "Fast/Acc")
    SessionLocal = account_session_makers[account_type]
    
    with SessionLocal() as db:
        # Get all completed/stopped trades ordered by end_time
        trades = db.query(TradeDataDB).filter(
            TradeDataDB.status.in_([TradeStatus.COMPLETED, TradeStatus.STOPPED])
        ).order_by(TradeDataDB.end_time.asc()).all()
        ...
        for trade in trades:
            ...
            trade_date = trade.end_time.strftime("%Y-%m-%d")
            ...
            date_stats[trade_date]["trade_count"] += 1
            
            if trade.profit_loss >= 0:
                date_stats[trade_date]["total_profit"] += trade.profit_loss
                date_stats[trade_date]["profitable_trades"] += 1
            else:
                date_stats[trade_date]["total_loss"] += abs(trade.profit_loss)
                date_stats[trade_date]["losing_trades"] += 1
            
            date_stats[trade_date]["net_profit_loss"] += trade.profit_loss
        ...
        for date_key in sorted(date_stats.keys()):
            ...
            stats["net_profit_loss"] = round(stats["net_profit_loss"], 2)
            result.append(stats)
        
        # Calculate overall totals
        overall_profit = sum(s["total_profit"] for s in result)
        overall_loss = sum(s["total_loss"] for s in result)
        overall_net = sum(s["net_profit_loss"] for s in result)
        total_trades = sum(s["trade_count"] for s in result)
        ...
        return {
            ...
            "overall_summary": {
                ...
                "net_profit_loss": round(overall_net, 2),
                "total_trades": total_trades
            },
            "timestamp": get_current_time_in_timezone().strftime(f"%Y-%m-%d %H:%M:%S {SYSTEM_TIMEZONE}"),
            "message": "Profits and losses by date retrieved successfully"
        }
...
class DepositRequest(BaseModel):
    amount: float = Field(0.0, ge=0.0, description="Deposit amount to add to the current account balance")

@app.post("/api/account/deposit")
async def deposit(request: DepositRequest = DepositRequest()):
    account_type = session_state.get("current_account", "Fast/Acc")
    account_metrics = account_metrics_store[account_type]
    
    old_balance = account_metrics.balance
    new_balance = old_balance + request.amount
    account_metrics.balance = new_balance
    account_metrics.deposit = request.amount
    await update_account_metrics(account_type)
    await manager.broadcast(json.dumps({"type": "deposit", "account_type": account_type, "amount": request.amount, "old_balance": old_balance, "new_balance": new_balance, "timestamp": get_current_time_in_timezone().strftime(f"%Y-%m-%d %H:%M:%S {SYSTEM_TIMEZONE}")}))
    print(f"Deposited {request.amount:.2f} to {account_type}: {old_balance:.2f} -> {new_balance:.2f}")
    return {"status": "success", "account_type": account_type, "message": f"Deposited {request.amount:.2f}", "old_balance": old_balance, "new_balance": new_balance}
...
class StartTradeRequest(BaseModel):
    ...
    starting_price: Optional[float] = Field(None, gt=0, description="Custom starting/opening price for trades (uses default if not provided)")
...
async def start_trade(symbol: str, request: StartTradeRequest):
    ...
    if request.starting_price is not None:
        # User provided custom starting/opening price - use it for both BUY and SELL
        if direction == TradeDirection.BUY:
            entry_price = request.starting_price
            current_buy_price = request.starting_price
            current_sell_price = request.starting_price + symbol_settings.spread
        else:  # SELL
            entry_price = request.starting_price
            current_sell_price = request.starting_price
            current_buy_price = request.starting_price - symbol_settings.spread
    else:
        # No custom price provided - use defaults from currency pair settings
...
async def websocket_endpoint(websocket: WebSocket, client_id: str, token: str = None):
    ...
    await websocket.send_text(json.dumps({"type": "connection_established", "client_id": client_id, "role": user_role, "is_admin": is_admin, "current_account": session_state.get("current_account", "Fast/Acc"), "timestamp": get_current_time_in_timezone().strftime(f"%Y-%m-%d %H:%M:%S {SYSTEM_TIMEZONE}")}))
...
@app.post("/api/profile/upload/{category}")
async def upload_profile_image(category: ImageCategory, file: UploadFile = File(...)):
    """
    Upload an image for a specific category (chats, messages, or quotes).
    Permanently deletes the previous image and replaces it with the new one.
    """
    # Validate file type
    ...
    with ProfileSessionLocal() as db:
        # Delete ALL existing images for this category (ensures complete removal)
        existing_images = db.query(ProfileImageDB).filter(
            ProfileImageDB.category == category.value
        ).all()
        
        if existing_images:
            for existing_image in existing_images:
                db.delete(existing_image)
            db.commit()  # Commit deletion first to ensure old data is removed
            action = "replaced"
        else:
            action = "uploaded"
    ...
    print(f"Profile image {action} for category '{category.value}': {image_id}")
    
    return {
        "status": "success",
        "message": f"Image {action} successfully for {category.value}",
        "category": category.value,
        "image_id": image_id,
        "action": action
    }

@app.get("/api/profile/image/{category}")
async def get_profile_image(category: ImageCategory):
    """
    Get the current image for a specific category.
    """
    with ProfileSessionLocal() as db:
        image = db.query(ProfileImageDB).filter(
            ProfileImageDB.category == category.value
        ).first()
        
        if not image:
            raise HTTPException(status_code=404, detail=f"No image found for category '{category.value}'")
...
@app.get("/api/profile/images/all")
async def get_all_profile_images():
    """
    Get all profile images for all categories.
    """
    with ProfileSessionLocal() as db:
        images = db.query(ProfileImageDB).all()
        ...
        return {
            "status": "success",
            "images": result,
            "message": f"Retrieved {len(result)} profile images"
        }

@app.delete("/api/profile/image/{category}")
async def delete_profile_image(category: ImageCategory):
    """
    Delete the image for a specific category.
    """
    with ProfileSessionLocal() as db:
        image = db.query(ProfileImageDB).filter(
            ProfileImageDB.category == category.value
    ...
    print(f"Profile image deleted for category '{category.value}': {image_id}")
    
    return {
        "status": "success",
        "message": f"Image deleted successfully for {category.value}",
        "category": category.value,
        "image_id": image_id
    }
...
if __name__ == "__main__":
    ...
    print("\nAvailable Accounts: Fast/Acc, Demo/Acc, Hunter/Acc, LivePro/Acc")
    print("="*60 + "\n")
    
    # Function to open the HTML dashboard after server starts (only for local development)
    def open_dashboard():
        # Skip opening browser on Render (production)
        if os.getenv('RENDER'):
            return
        ...
        if os.path.exists(html_file):
            ...
            print(f"WARNING: Dashboard file not found: {html_file}")
    
    # Start timer to open HTML dashboard (only for local development)
    if not os.getenv('RENDER'):
        Timer(0.5, open_dashboard).start()
    
    try:
        # Get port from environment variable for production (Render), default to 8080 for local development
